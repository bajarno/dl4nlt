...
class S2SEncoder(nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        # same ordering. Inputs are expected to be sorted for decoder, and encoder sorts/unsorts
        # internally.
        if lengths is not None:
            lengths_sorted, sort_idx = lengths.sort(0, descending=True)
            unsort_idx = sort_idx.sort()[1]
            x = x[sort_idx]
    ...
    def _get_forward_hidden(self, hidden):
        ...
        h_forward = h_e.view(enc_layers, 2, enc_batch, enc_hidden)[:,0].squeeze(1)
        c_forward = c_e.view(enc_layers, 2, enc_batch, enc_hidden)[:,0].squeeze(1)
...
if __name__=="__main__":
    ...
    # Encoder Inputs
    X = torch.LongTensor(batch_size, encoder_maxlen).random_(10)
    X_lengths = torch.LongTensor([12, 6])
    # Decoder inputs
    d_in = torch.LongTensor(batch_size, 1).random_(10)
    d_lengths = torch.LongTensor([1, 1])

    # Model
    e = S2SEncoder(vocab_size, embedding_size, hidden_size, num_layers)
    d = S2SAttnDecoder(vocab_size, embedding_size, hidden_size, num_layers)


    enc_outputs, h = e.forward(X, X_lengths)
    print("ENCODER in,out,h", X.size(), enc_outputs.size(), h[0].size())
    #decoder_inputs, decoder_lengths, prev_hidden, encoder_outputs, encoder_lengths
    d_out, d_h = d(d_in, d_lengths, h, enc_outputs, X_lengths)
    print("DECODER in,out,h", d_in.size(), d_out.size(), d_h[0].size())